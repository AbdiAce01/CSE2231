...
public final class JCFExplorations {
    ...
    public static void giveRaise(components.map.Map<String, Integer> map,
        ...
        while (map.size() > 0) {
            Pair<String, Integer> Employee = map.removeAny();
            if (Employee.key().charAt(0) == initial) {
                int value = Employee.value()
                        + ((raisePercent * Employee.value()) / 100);
                Newmap.add(Employee.key(), value);
            } else {
                Newmap.add(Employee.key(), Employee.value());
            }
        }
    ...
    public static void giveRaise(java.util.Map<String, Integer> map,
        ...
        for (java.util.Map.Entry<String, Integer> newMap : map.entrySet()) {
            if (newMap.getKey().toString().charAt(0) == initial) {
                newMap.setValue(newMap.getValue()
                        + (newMap.getValue() * raisePercent) / 100);
            }
        }
